...
description: "Playwright browser engine with proxy escalation for reliable scraping."
---
...
```
Attempt 1: Playwright on datacenter proxy (10s timeout)
    ↓ any failure (timeout, empty, blocked, error)
Attempt 2: Playwright on residential proxy (remaining time, up to 30s total)
    ↓ any failure
    Done - report error
```
