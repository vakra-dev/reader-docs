...
Your account has **60 RPM** and **10 concurrent jobs**. 60 RPM means one request per second, every second. If your pipeline can burst above that, you'll get 429s. Design for the average, not the peak.
...
await reader.read({ urls }); // creates a job, SDK polls to completion
```
...
1. **Two API keys, two rate limit overrides**: set a low RPM override on the background key so interactive traffic always has room.
