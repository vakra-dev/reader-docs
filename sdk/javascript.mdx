...
const client = new ReaderClient({ apiKey: "rdr_..." });
...
| Method | Description |
|--------|-------------|
| `client.sessions.create(params?)` | Create a session, returns `SessionInfo` with `wsEndpoint` |
| `client.sessions.get(sessionId)` | Get session status |
| `client.sessions.stop(sessionId)` | Stop a session, returns billing summary |
