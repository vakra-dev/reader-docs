...
Sites that hydrate dynamically *and* detect bots are harder still. `waitForSelector` gets you past the hydration; `proxyMode: "premium"` gets you past the bot detection. They work independently, and you might need both:

```ts
await reader.read({
  url: "https://shop.example.com/item/42",
  waitForSelector: ".product-price",
  proxyMode: "premium", // residential proxies bypass bot detection
});
```
...
4. **Check if the page needs premium mode.** Bot-walled pages may never fully hydrate for standard-mode traffic.
